...
void inicializaLexer(Lexer& lex)
{
	...
	//automata de !=
	dfaAddTransition(lex.dfa_diferente,0,'!',1);
	dfaAddTransition(lex.dfa_diferente,1,'=',2);
	dfaAddFinalState(lex.dfa_diferente,2);
...
Token getNextToken(Lexer& lexer,std::ifstream& inputStream,int cont_sl)
{
	...
	while(inputStream.good())
	{
		...
		//ejecuta el DFA de !=
		cad=dfaStart(lexer.dfa_diferente,inputStream);
		if(cad!="No aceptado")
		{
			token.nombre="OpeRel";
			token.atributo=cad;
